  ...
  return (
    <div className="px-8 py-8 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-xl font-semibold text-zinc-900">Competitors</h1>
          <p className="text-sm text-zinc-400 mt-0.5">Monitor website changes and updates</p>
        </div>
        <AddCompetitorModal />
      </div>
...
function StatCard({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className="bg-white rounded-xl border border-zinc-200/80 px-5 py-4">
      <p className="text-[11px] font-medium text-zinc-400 uppercase tracking-wider mb-1">{label}</p>
      <p className={`text-2xl font-semibold ${highlight ? 'text-indigo-600' : 'text-zinc-900'}`}>{value}</p>
    </div>
  )
}
    ...
    <div className="bg-white rounded-xl border border-zinc-200/80 p-14 text-center">
      <div className="w-12 h-12 rounded-xl bg-zinc-100 flex items-center justify-center mx-auto mb-4">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="8" cy="8" r="6" stroke="#a1a1aa" strokeWidth="1.5"/>
          <circle cx="12" cy="12" r="6" stroke="#a1a1aa" strokeWidth="1.5"/>
        </svg>
      </div>
      <h3 className="text-base font-semibold text-zinc-900 mb-1">No competitors yet</h3>
