    ...
    <div className="min-h-screen bg-[#07101f] flex items-center justify-center p-4">
      <div className="w-full max-w-[360px]">
        ...
        <div className="bg-[#0b1628] rounded-2xl border border-[#182b45] p-8">
          <h1 className="text-[18px] font-semibold text-[#dce8ff] mb-1">{t('signUp.title')}</h1>
          <p className="text-[13px] text-[#4d6a8a] mb-6">{t('signUp.subtitle')}</p>

          {error && <div className="mb-5 px-3 py-2.5 bg-red-950/50 border border-red-800/50 rounded-lg text-red-400 text-[13px]">{error}</div>}
          {success && <div className="mb-5 px-3 py-2.5 bg-emerald-950/50 border border-emerald-700/50 rounded-lg text-emerald-400 text-[13px]">{success}</div>}

          <form action={signUp} className="space-y-4">
            <div>
              <label htmlFor="email" className="block text-[12px] font-medium text-[#6b85aa] mb-1.5">{t('signUp.email')}</label>
            ...
            <div>
              <label htmlFor="password" className="block text-[12px] font-medium text-[#6b85aa] mb-1.5">{t('signUp.password')}</label>
              <input id="password" name="password" type="password" required autoComplete="new-password" minLength={6} placeholder="••••••••"
                className="w-full px-3 py-2.5 border border-[#182b45] rounded-lg text-[13px] text-[#dce8ff] bg-[#071018] placeholder-[#2d4a68] focus:outline-none focus:ring-2 focus:ring-[#4f74ff]/30 focus:border-[#4f74ff] transition-colors" />
              <p className="text-[11px] text-[#364f6e] mt-1.5">{t('signUp.passwordHint')}</p>
            </div>
            <button type="submit" className="w-full bg-[#4f74ff] hover:bg-[#3d63ee] text-white py-2.5 rounded-lg text-[13px] font-medium transition-colors mt-2">
              {t('signUp.submit')}
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-[12px] text-[#4d6a8a] mt-5">
          {t('signUp.hasAccount')}{' '}
          <Link href="/auth/sign-in" className="text-[#4f74ff] hover:text-[#7a96ff] font-medium transition-colors">{t('signUp.signInLink')}</Link>
        </p>
