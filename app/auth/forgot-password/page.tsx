    ...
    <div className="min-h-screen bg-[#f7f8fa] flex items-center justify-center p-4">
      <div className="w-full max-w-[360px]">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200/80 shadow-sm p-8">
          <h1 className="text-[18px] font-semibold text-zinc-900 mb-1">{t('forgotPassword.title')}</h1>
          <p className="text-[13px] text-zinc-400 mb-6">{t('forgotPassword.subtitle')}</p>

          {error && <div className="mb-5 px-3 py-2.5 bg-red-50 border border-red-100 rounded-lg text-red-600 text-[13px]">{error}</div>}

          {success ? (
            <div className="px-3 py-2.5 bg-emerald-50 border border-emerald-100 rounded-lg text-emerald-700 text-[13px]">{success}</div>
          ) : (
            <form action={resetPassword} className="space-y-4">
              <div>
                <label htmlFor="email" className="block text-[12px] font-medium text-zinc-700 mb-1.5">{t('forgotPassword.email')}</label>
